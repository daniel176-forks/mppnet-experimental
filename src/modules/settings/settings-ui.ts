	...
	if (window.location.hostname === 'multiplayerpiano.com') {
		...
		const showSynth = () => {
				...
				setting.onclick = () => {
					...
					if (settings.smoothCursor) {
						document
							...
							.setAttribute('smooth-cursors', '');
					} else {
						document
							.getElementById('cursors')!
							.removeAttribute('smooth-cursors');
					}
					if (settings.smoothCursor) {
			...
			(() => {
				...
				setting.className = 'setting';
				(setting as HTMLElement).style.cssText =
					'color: inherit; width: calc(100% - 2px);';
		...
		const changeClientSettingsTab = (
			...
			switch (tabName.toLowerCase()) {
				...
				case 'piano': {
					...
					const selectSetting = document.createElement(
						'select',
						...
						() => {
							...
							if (settings.smoothCursor) {
								document
									.getElementById('cursors')!
									.setAttribute('smooth-cursors', '');
								Object.values(client.ppl).forEach(
							...
							Object.values(client.ppl).forEach((participant: Participant) => {
								if (
									...
									if (settings.hideBotUsers) {
										const nd = document.getElementById(
											'namediv-' + participant.id,
										);
		...
		document
			.querySelectorAll('.client-settings-tablink[data-tab]')
			.forEach(el => {
				el.addEventListener('click', e => {
					changeClientSettingsTab(
						{ currentTarget: e.currentTarget as Element },
						(el as HTMLElement).dataset.tab!,
					);
				});
			});
